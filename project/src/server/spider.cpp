...
void LoadLibTest()
{
	...
	char *error;
	pFunction pfun = NULL;
	handle = dlopen(MY_LIB_SO, RTLD_LAZY);
	if (handle == NULL)
	{
		fprintf(stderr, "%s\n", dlerror());
		exit(EXIT_FAILURE);
	...
	dlerror();
	//*(void **) (&pfun) = dlsym(handle, "test");
	pfun =	(pFunction)dlsym(handle, "test");
...
int main(int argc, char *argv[])
{
    ...
    /* 载入模块*/
    for( auto it = g_conf->modules.begin(); it != g_conf->modules.end(); it++) {
        dso_load(g_conf->module_path, it->c_str());
    } 
